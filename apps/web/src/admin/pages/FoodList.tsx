...
export default function FoodList({ token }: { token: string }) {
  const navigate = useNavigate();
  const [statusFilter, setStatusFilter] = useState<string>("");
  const [searchText, setSearchText] = useState("");
  const { data: merchants, loading, reload } = useAdminData<Dict[]>(
    token,
    "/api/admin/merchants"
  );
  ...
  const foodMerchants = (merchants || []).filter(
    (m) => m.categoryId === foodCategory?.id || m.category?.slug === "food"
  );

  const filteredMerchants = foodMerchants.filter((merchant) => {
